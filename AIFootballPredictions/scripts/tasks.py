...
def data_acquisition(c, leagues="E0 I1 SP1 F1 D1", seasons="2425 2324 2223", raw_data_output_dir="data/raw"):
    ...
    c.run(f"python3 scripts/data_acquisition.py --leagues {leagues} --seasons {seasons} --raw_data_output_dir {raw_data_output_dir}")
...
def data_preprocessing(c, raw_data_input_dir="data/raw", processed_data_output_dir="data/processed", num_features=20, clustering_threshold=0.5):
    ...
    c.run(f"python3 scripts/data_preprocessing.py --raw_data_input_dir {raw_data_input_dir} --processed_data_output_dir {processed_data_output_dir} --num_features {num_features} --clustering_threshold {clustering_threshold}")
...
def train_models(c, processed_data_input_dir="data/processed", trained_models_output_dir="models", metric_choice="accuracy", n_splits=10, voting="soft"):
    ...
    c.run(f"python3 scripts/train_models.py --processed_data_input_dir {processed_data_input_dir} --trained_models_output_dir {trained_models_output_dir} --metric_choice {metric_choice} --n_splits {n_splits} --voting {voting}")
...
def acquire_next_matches(c, get_teams_names_dir="data/processed", next_matches_output_file="data/next_matches.json"):
    ...
    c.run(f"python3 scripts/acquire_next_matches.py --get_teams_names_dir {get_teams_names_dir} --next_matches_output_file {next_matches_output_file}")
...
def make_predictions(c, models_dir="models", data_dir="data/processed", output_file="telegram_post.txt", json_competitions="data/next_matches.json"):
    ...
    c.run(f"python3 scripts/make_predictions.py --input_leagues_models_dir {models_dir} --input_data_predict_dir {data_dir} --final_predictions_out_file {output_file} --next_matches {json_competitions}")
